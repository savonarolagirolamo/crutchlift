 ...
 *   }
 */
export function readCache (config: VendeeConfig): Record<string, Metadata> {
 ...
 *   }
 */
export function writeCache (config: VendeeConfig, metadata: Record<string, Metadata>): void {
  const file = path.resolve(process.cwd(), config.paths.cache, FILE)
